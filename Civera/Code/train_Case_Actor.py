...
X_train = case_index_not_null[columns]
print(X_train.head())

# # cdocs_case_action_index / actor = null
# actor_null = pd.read_sql("SELECT * FROM wp_courtdocs.cdocs_case_action_index as c_a_index WHERE c_a_index.action != ' ' and c_a_index.actor = ' ' and rand() <= .2;", con = mydb)
# testSet = actor_null[columns]
# print(testSet.head())
...
r = re.compile(r'[^\w\s]+')
X_train['description'] = [r.sub('', x) for x in X_train['description'].tolist()]
X_train['description'] = X_train['description'].str.lower().str.split()

stopwords = stopwords.words('english')
X_train['description'] = X_train['description'].apply(lambda x: [item for item in x if item not in stopwords])
print("stopwords")
print(X_train.head())

lemmatizer = WordNetLemmatizer() 
X_train['description'] = X_train['description'].apply(lambda x:[lemmatizer.lemmatize(word) for word in x])

#remove duplicate words after lemmatizing 
X_train['description'] = X_train['description'].apply(lambda x:list(dict.fromkeys(x)))
print()
print('trainingSet after lemmatizer & removing dupes ')
print(X_train.head())
print('processing -remove dupes')
